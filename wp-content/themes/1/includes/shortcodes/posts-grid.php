		...
		switch ($link) {
			...
			foreach ( $posts as $j => $post ) {
				...
				$output .= '<li class="'. $spans .' list-item-'.$count.'">';
					...
					if($i % 2 == 1 && $custom_class == 'services') {
					...
					if($custom_class == 'posts-home' && has_post_thumbnail($post_id)) {
						...
						$output .= '<div class="thumb-desc"><div class="thumb-desc-in"><h5>'.get_the_title($post_id).'</h5>';
						$output .= '<span class="post_category">';
							if ($type!='' && $type!='post') {
								$terms = get_the_terms( $post_id, $type.'_category');
								if ( $terms && ! is_wp_error( $terms ) ) {
						...
						$output .= '</span></div><div class="helper"></div></div>';
					...
					if($i % 2 == 0 || $custom_class !== 'services') {
							...
							if ($meta == 'yes') {
								...
								$output .= '<div class="post_meta">';
									...
									if (isset($cat_data['img'])){
										$output .= '<div class="category_icon"><i class="'.$cat_data['img'].'"></i></div>';
									}*/
								$output .= '</div>';
								// end post meta
							}
