...
if ( !function_exists( 'banner_shortcode' ) ) {
	...
	function banner_shortcode( $atts, $content = null, $shortcodename = '' ) {
		...
		$output =  '<div class="banner-wrap '.$custom_class.' clearfix">';
		if ($img !="") {
			$output .= '<figure class="featured-thumbnail">';
			if ($banner_link != "") {
				$output .= '<a href="'. $banner_link .'" title="'. $title .'"><img src="' . $img .'" title="'. $title .'" alt="" /><div class="helper"></div></a>';
			} else {
				$output .= '<div class="desc-in"><img src="' . $img .'" title="'. $title .'" alt="" /><div class="helper"></div></div>';
			}
			$output .= '</figure>';
		}
